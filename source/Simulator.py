...
class Simulator:
    ...
    def run(self, site: Polygon) -> list[tuple[str, Polygon, tuple[float, float], float]]:
        polygonQueue: Queue[Polygon] = Queue()
        polygonQueue.put(site)
        # list[tuple[sampleId, sitePolygon, translationXY, ccwRotation]]
        generation: list[tuple[str, Polygon, tuple[float, float], float]] = list()
        while not polygonQueue.empty():
            polygon: Polygon = polygonQueue.get()
            generated: list[tuple[str, Polygon, tuple[float, float], float]]
    ...
    def generate(self, polygon: Polygon) ->\
        tuple[list[tuple[str, Polygon, tuple[float, float], float]], list[Polygon]]:
        generators: list[tuple[str, Polygon]] = self.findGenerators(polygon)
        generated: list[tuple[str, Polygon, tuple[float, float], float]] = list()
        ...
        for generator in generators:
            ...
            similarPoint: Point = similarPerception.getPoint()
            sitePoint: Point = sitePerception.getPoint()
            translation: tuple[float, float] = (sitePoint.x - similarPoint.x, sitePoint.y - similarPoint.y)
            generated.append((similarPerception.getId(), generatedPolygon, translation, rotation))
