...
class Collection:
    ...
    @classmethod
    def fromIdsPointsRegionsSamples(
        cls,
        ids: Sequence[str],
        points: Sequence[shapely.Point],
        regions: Sequence[shapely.Polygon],
        samples: CollectionType[Sample],
    ) -> Self:
    ...
    def query(
        ...
        target: Attributes,
    ) -> tuple[Perception, float, list[shapely.Polygon], Attributes, Buildings]:
        print(f"Querying {self.__repr__()} with {query}")
        perceptionRotations: list[tuple[Perception, float]] = (
            self.findRotations(query)
        )
        destination: tuple[float, float] = (
            query.getPoint().x,
            query.getPoint().y,
        )
        perceptionStats: list[
            tuple[
                float,
                Perception,
                float,
    ...
    def filter(self, sitePolygons: list[shapely.Polygon]) -> Self:
        sitePerceptionZones: list[shapely.Polygon] = (
    ...
    def update(
        ...
        points: list[shapely.Point] = [
            perception.getPoint() for perception in self.perceptions
        ]
        regions: list[shapely.Polygon] = [
            perception.getRegion() for perception in self.perceptions
        ]
    ...
    def perceptionFromSample(self, sample: Sample) -> Perception:
        matchingPerceptions: list[Perception] = list(
            filter(
