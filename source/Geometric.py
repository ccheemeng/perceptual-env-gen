...
class Geometric:
    ...
    @staticmethod
    def voronoiPolygons(
        points: list[shapely.Point], extendTo: Optional[shapely.Geometry] = None
    ...
    @staticmethod
    def translate(
        geometry: object, originOrVector: object, destination: object = None
    ) -> object:
        if destination == None:
    ...
    def translateVector(geometry: object, vector: object) -> object:
        ...
        raise ValueError(
            f"Vector {vector} must be " "shapely.Point or tuple[float, float]!"
        )

    @staticmethod
    def translateOD(
        geometry: object, origin: object, destination: object
    ) -> object:
        if isinstance(origin, shapely.Point) and isinstance(
        ...
        if isinstance(origin, shapely.Point) and isinstance(
            ...
            return Geometric.translateVectorTuple(
                geometry, (destination[0] - origin.x, destination[1] - origin.y)
            )
        if isinstance(origin, Sequence) and isinstance(
            destination, shapely.Point
        ):
            Geometric.checkSequence2Float(origin)
            return Geometric.translateVectorTuple(
                geometry, (destination.x - origin[0], destination.y - origin[1])
            )
    ...
    def translateVectorTuple(
        geometry: object, vector: tuple[float, float]
    ) -> object:
    ...
    @staticmethod
    def translatePoint(
        geometry: shapely.Point, vector: tuple[float, float]
    ) -> shapely.Point:
    ...
    def translatePolygon(
        geometry: shapely.Polygon, vector: tuple[float, float]
    ) -> shapely.Polygon:
    ...
    @staticmethod
    def translateTuple(
        geometry: tuple[float, float], vector: tuple[float, float]
    ) -> tuple[float, float]:
    ...
    def rotateAboutShapely(
        geometry: object, origin: shapely.Point, rotation: float
    ) -> object:
    ...
    def rotateAboutTuple(
        geometry: object, origin: tuple[float, float], rotation: float
    ) -> object:
        if isinstance(geometry, shapely.Point):
    ...
    @staticmethod
    def rotatePoint(
        geometry: shapely.Point, origin: tuple[float, float], rotation: float
    ) -> shapely.Point:
        return shapely.Point(
    ...
    @staticmethod
    def rotatePolygon(
        geometry: shapely.Polygon, origin: tuple[float, float], rotation: float
    ) -> shapely.Polygon:
        exterior: list[shapely.Point] = [
