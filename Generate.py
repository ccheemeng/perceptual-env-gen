...
def main(args: argparse.Namespace) -> None:
    ...
    for run in runs:
        ...
        with open(path.join(args.gen_dir, run, "perceptions.csv"), "r") as fp:
            transformationDf = pd.read_csv(fp, header=0, index_col=0).rename(
                index=(lambda i: str(i))
            )
        multiPolygonGdf: geopandas.GeoDataFrame
        with open(path.join(args.gen_dir, run, "polygons.geojson"), "r") as fp:
            multiPolygonGdf = geopandas.read_file(fp)
            multiPolygonGdf = multiPolygonGdf.set_index("id", drop=True).rename(
                index=(lambda i: str(i))
            )
        index: str
        ...
        for index, multiPolygonSeries in multiPolygonGdf.iterrows():
            ...
            pointsGdf: geopandas.GeoDataFrame = geopandas.GeoDataFrame(
                ...
                Geometric.rotateAboutTuple(
                    Geometric.translateVectorTuple(multiPolygon, translation),
                    destination,
                    rotation,
                ),
                keep_geom_type=True,
