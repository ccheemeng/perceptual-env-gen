...
def main(args: argparse.Namespace) -> None:
    queryCollection: Collection = IO.initCollection(
        args.query[0], args.query[1], args.query[2])
    siteCollection: Collection = IO.initCollection(
        args.site[0], args.site[1], args.site[2])
    sitePolygons: list[
        tuple[str, shapely.Polygon, Attributes]
    ] = IO.initPolygons(args.polygons, args.target)
    queryBuildings: Buildings = IO.initBuildings(args.buildings)
    siteCollection = siteCollection.filter(
        [polygon for id, polygon, attributes in sitePolygons])
    simulator: Simulator = Simulator(queryCollection, queryBuildings)
    generations: list[tuple[str, list[tuple[
        Perception,
        shapely.Point,
        float,
    ...
    for id, sitePolygon, siteAttributes in sitePolygons:
        generations.append(
            (id, simulator.run(sitePolygon, siteAttributes, siteCollection)))
    generation: tuple[str, list[tuple[
        Perception,
        shapely.Point,
        float,
        tuple[shapely.Polygon, ...],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "The following attributes will be taken "
            "from feature attributes if present:\n"
            "(1) height\n"
            "(2) residential_gfa\n"
            "(3) commercial_gfa\n"
            "(4) civic_gfa\n"
